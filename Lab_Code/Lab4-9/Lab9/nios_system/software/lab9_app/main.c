...
#define to_hw_port (volatile char*) 0x2040
#define to_hw_sig (volatile char*) 0x2020
#define to_sw_port (char*) 0x2030
#define to_sw_sig (char*) 0x2010
...
/*Function: PrintState - prints the given input state as a matrix*/
...
word SubWord(word w) {
	...
	for (; i < 4; i++) {
		temp = w >> (24 - 8*i) & 0xFF;
		ret += (aes_sbox[ temp >> 4][temp & 0xF]) << (24 - 8*i);
	}
	return ret;
}
...
void KeyExpansion(byte* key, word* w, int Nk) {
	...
	for (; i < Nk; i++)
		w[i] = (word)((key[4*i] << 24) + (key[4*i + 1] << 16) + (key[4*i + 2] << 8) + key[4*i + 3]);

	for (i = Nk; i < 44; i++){
		temp = w[i-1];
		if ((i % Nk) == 0) {
			//sudocode: temp = SubWord(RotWord(temp)) xor Rcon[i/Nk]
			temp = SubWord((temp << 8) + ((temp & 0xFF000000) >> 24)) ^ Rcon[i/Nk -1];
		}
		w[i] = w[i-Nk] ^ temp;
	}
}
...
/* Function: SubBytes()
*  Inputs:  "byte * b" - byte array that needs to be substituted
*  Outputs: None
*  Effects: Updates "byte * b" to be appropriate value based off of sbox
*/
void SubBytes(byte * b){
   int i = 0;
   for (; i < 16; i++)
	  //do some logic to get rows and columns to lookup in sbox
      b[i] = aes_sbox[b[i] >> 4][b[i] & 0xF];
}
...
*  Effects:
*/
void ShiftRow(byte* shift_row, int num_shift) {
  byte temp[4];
  int i;

  //copy input shift_row array (to make it easier)
  for (i = 0; i < 4; i++) {
    temp[i] = shift_row[i];
  }

  //Shift each value in given row by value in parameter
  for (i = 0; i < 4; i++) {
    shift_row[(((i - num_shift)%4)+4)%4] = temp[i];
  }
}
...
void ShiftRows(byte* state) {
  ...
  //loop through each row and shift by 0,1,2,or 3
  for (i = 0; i < 4; i++) {
    ShiftRow(&state[i*4], shift);
    shift++;
  }
}
...
void MixColumns(byte* state) {
  int i, j;
  for (i = 0; i < 4; i++) {
	byte a[4] = {state[i], state[i+4], state[i+8], state[i+12]};

	byte b0, b1, b2, b3;
	byte xtime[4];

	for (j = 0; j < 4; j++)
		xtime[j] = (a[j] << 1) ^ (0x1B & (unsigned char)((signed char)a[j] >> 7));

	b0 = xtime[0] ^ (xtime[1] ^ a[1]) ^ a[2] ^ a[3];
	b1 = a[0] ^ xtime[1] ^ (xtime[2] ^ a[2]) ^ a[3];
	b2 = a[0] ^ a[1] ^ xtime[2] ^ (xtime[3] ^ a[3]);
	b3 = (xtime[0] ^ a[0]) ^ a[1] ^ a[2] ^ xtime[3];
...
void AddRoundKey(byte* state, word* round_key) {
	byte round_key_bytes[16];
	int i, j;

	//loop through all values in round key and
	for (i = 0; i < 4; i++) {
		for (j = 0; j < 4; j++) {
			round_key_bytes[(j * 4) + i] = (round_key[i] >> (24-(8*j))) & 0xFF;
		}
	}

	//XOR state matrix and round key (given your position in array)
	for (i = 0; i < 16; i++) {
		state[i] = state[i] ^ round_key_bytes[i];
	}
}



/* Function: AES Main Implementation
*  Input:
*  Output:
*  Effects:
*  Pseudocode:
...
*  end
*/
void AES(byte* in, byte* out, round_key* round_keys) {
   int round;
   byte* state = out;
   memcpy(state, in, 16);
   AddRoundKey(state, round_keys[0].words);

   //ROUNDS 1-9
   for (round = 1; round < 10; round++) {
		#if DEBUG_PRINT
	   printf("Initial state at round %d...\n", round);
	   printState(state);
	   printf("\n");
		#endif
	   SubBytes(state);
	   ...
	   AddRoundKey(state, round_keys[round].words);
   }
   //ROUND 10
   SubBytes(state);
   ShiftRows(state);
   AddRoundKey(state, round_keys[10].words);
   out = state;
}
 ...
 */
void encrypt(unsigned char plain_text[32], unsigned char key[32], unsigned char* result) {
	word w[44];
	unsigned char col_text[16];
	int i, x, y;
	round_key round_keys[11];

	//Turn key string into hex
	for(i = 0; i < 32; i+=2) {
		key[i/2] = charsToHex(key[i], key[i+1]);
	}

	KeyExpansion(key, w, 4);

	//copy key expansion over to round_keys
	for (i = 0; i < 11; i++)
		round_keys[i].words = &w[i*4];

	//Turn regular text into hex
	for(i = 0; i < 32; i+=2) {
		plain_text[i/2] = charsToHex(plain_text[i], plain_text[i+1]);
	}

	//Organize plain text into column_text so we can put it into AES
	for (y = 0; y < 4; y++) {
		for (x = 0; x < 4; x++) {
			col_text[x*4 + y] = plain_text[x + y*4];
		}
	}

	//Perform AES Encryption on plaintext with key
	AES(col_text, result, round_keys);
}
...
int main() {
	...
	while(1) {
		...
		scanf("%s", plaintext);
		strncpy(plaintext, "ece298dcece298dcece298dcece298dc", 33); // for testing purposes

		//Get key from user
		printf("Enter key: ");
		scanf("%s", key);
		strncpy(key, "000102030405060708090a0b0c0d0e0f", 33); // for testing purposes

		//Encrypt message
		printf("Encrypting %s using key %s.\n", plaintext, key);
		...
		for (y = 0; y < 4; y++) {
			for (x = 0; x < 4; x++) {
				printf("%x%x", (ciphertext[x*4 + y] >> 4), (ciphertext[x*4 + y] & 0xF));
			}
		}
		...
		int i =0;
		byte* encryptedMsg = ciphertext;
        for (y = 0; y < 4; y++) {
       	 for (x = 0; x < 4; x++) {
       	     int packet = encryptedMsg[x*4 + y];
                *to_hw_sig = 1; // tell hardware we are sending the message
                *to_hw_port = packet; // send 8 bits to hardware
                while (*to_sw_sig != 1); // wait for hardware to Ack
                *to_hw_sig = 2; // notify hardware we've received Ack
                while (*to_sw_sig != 0); // wait for hardware to get ready for next message
            }
        }
        *to_hw_sig = 0;
        printf("Transmission complete.\n");

        // Transmit key to hardware side for decryption
        printf("Transmitting key...\n");
        for (i = 0; i < 16; i++) {
       	 unsigned char packet = key[i];
            *to_hw_sig = 2;
            *to_hw_port = packet;
            while (*to_sw_sig != 2);
            *to_hw_sig = 1;
        ...
        *to_hw_sig = 3;
		...
		for (i = 0; i < 16; i++) {
			*to_hw_sig = 1;
			while (*to_sw_sig != 1);
			plaintext[i] = *to_sw_port;
			*to_hw_sig = 2;
			while (*to_sw_sig != 0);
		}
		printf("Completed receiving message.\n");
		printf("Decoded message: ");
		for (i = 15; i >= 0; i--) {
			printf("%x%x", plaintext[i] >> 4, plaintext[i] & 0xF);
		}
		printf("\n");
	}
   return 0;
