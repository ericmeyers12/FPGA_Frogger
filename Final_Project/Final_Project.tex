...
\section{Purpose of Modules}
	...
	\subsection{frog}
	The frog module contains the logic for the moving component controlled by the user (the ``frog"). This entails the logic for what the frog does when detecting it is colliding with a car, a lilypad, water, or the end position. The frog is implemented as a state machine that depending on the actions of the user, will move to the cooresponding state. The frog state diagram is explained below in the "Finite State Machines" section. \\
	
	This module takes inputs as the desired start position of the frog, the controller keys (up, down, left, right), lilypad collision signals, car collision signals, lilypad parameters (so it can sync up with the movements of the lilypad when colliding), and the win/lose signal. The module outputs the frog x and y position to display on the screen. Depending on the state of these win/lose signals the frog stops responding to input. 
